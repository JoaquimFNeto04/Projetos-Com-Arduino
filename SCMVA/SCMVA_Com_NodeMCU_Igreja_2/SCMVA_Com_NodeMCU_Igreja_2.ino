...
double contaPulso; //Variável para a quantidade de pulsos
...
double contaPulso2; //Variável para a quantidade de pulsos
...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(interruptPin), incpulso, CHANGE); //Configura o pino D1(Interrupção 0) para trabalhar como interrupção
  attachInterrupt(digitalPinToInterrupt(interruptPin2), incpulso2, CHANGE); //Configura o pino D2(Interrupção 0) para trabalhar como interrupção
...
void leitura1 () {
  ...
  Serial.print("CONTA PULSO:  "); //Imprime L/min
  Serial.println(contaPulso);
  
  vazao = vazao + contaPulso; // 5.5; //Converte para L/min
